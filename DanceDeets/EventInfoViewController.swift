...
class EventInfoViewController: UICollectionViewController, UIGestureRecognizerDelegate {
    ...
    override func viewDidLoad() {
        ...
        let shareButton = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.Action, target: self, action: #selector(EventInfoViewController.shareButtonTapped(_:)))
    ...
    override func viewDidLayoutSubviews() {
        ...
        if let event = startEvent {
            ...
            if let index = events.indexOf(event) {
                ...
                CLSNSLogv("%@", getVaList(["Event \(event.id) not in EventInfoViewController.events"]))
            }
            startEvent = nil
        }
    }
